...
{
    Schema_of_SignUp.findOne({"email_in_database" : req.cookies.email},{following:1},function(err,doc)
    {
        if(err)
        {
            console.log("Error In Saving");
        ...
        else
        {
           ...
           // console.log(doc.following.length);
            for(var i =0;i<doc.following.length;i++)
            {
                email[i] = doc.following[i].email;
                 post[count] = Status.find({user_email: email[i]});
                count++;
               /* Status.find({user_email: email[i]},function(err,docs)
                {
                    if(err)
                    {console.log("Eoor");}
                    else
